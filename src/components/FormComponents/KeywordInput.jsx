...
const KeywordInput = ({ control, error, onKeywordChange }) => {
  ...
  const handleDeleteTag = (index) => {
    onKeywordChange((prevKeywords) => prevKeywords.filter((_, i) => i !== index));
  };

  useEffect(() => {
    // `control`에서 `getValues` 호출 전에 `control`이 유효한지 확인
    if (control && control.getValues) {
      const fieldValue = control.getValues('newKeywords');
      if (fieldValue && fieldValue !== newKeywords) {
  ...
  return (
    <div style={{
      ...
      <Controller
        ...
        control={control}
        render={({ field }) => {
          // field.value가 변경되었을 때, newKeywords 업데이트
          useEffect(() => {
            if (field.value && field.value !== newKeywords) {
              setNewKeywords(field.value);
          ...
          return (
            <input
              ...
              onChange={(e) => {
                setInputValue(e.target.value);
                field.onChange(e.target.value); // react-hook-form과 동기화
              }}
