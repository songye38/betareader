...
// 로컬 스토리지에 상태 저장하기
if (typeof window !== 'undefined') {
  // manuscript 상태가 변경될 때마다 localStorage에 저장
  useManuscriptStore.subscribe(
    (state) => {
      localStorage.setItem("manuscript", JSON.stringify(state.manuscript));
    },
    (state) => state.manuscript // manuscript 상태를 감지하여 저장
  );

  // localStorage에서 manuscript 불러오기
  const loadManuscriptFromLocalStorage = () => {
    const manuscript = localStorage.getItem("manuscript");
    return manuscript ? JSON.parse(manuscript) : { id: null };
  };

  // 초기 상태 설정 (localStorage에서 불러오기)
  const manuscript = loadManuscriptFromLocalStorage();

  // localStorage에서 불러온 manuscript로 상태 초기화
  useManuscriptStore.setState({
    manuscript: manuscript,
  });
}
