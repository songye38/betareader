...
const useTabStore = create((set) => ({
  ...
  setTabs: (newTabs, targetTabId = null) =>
    set(() => {
      const updatedTabs = newTabs.map((tab, index) => ({
        ...tab,
        selected: targetTabId ? tab.tab_id === targetTabId : index === 0,
      }));

      const selectedTab = targetTabId
        ? updatedTabs.find((tab) => tab.selected)
        : updatedTabs[0] || { tab_id: null, tab_no: null, id: null };

      return {
        tabs: updatedTabs,
        selectedTab: { tab_id: selectedTab.tab_id, tab_no: selectedTab.tab_no, id: selectedTab.id },
        currentManuscriptId: updatedTabs.length + 1,
      };
    }),
...
// 🛠 localStorage 버전으로 저장 / 복구
if (typeof window !== 'undefined') {
  // selectedTab 상태 변경 감지 → localStorage 저장
  useTabStore.subscribe(
    (state) => {
      localStorage.setItem("selectedTab", JSON.stringify(state.selectedTab));
    },
    (state) => state.selectedTab
  );

  // localStorage에서 selectedTab 불러오기
  const loadSelectedTabFromLocalStorage = () => {
    const selectedTab = localStorage.getItem("selectedTab");
    return selectedTab ? JSON.parse(selectedTab) : { tab_id: null, tab_no: null, id: null };
  };

  // 초기화: localStorage에 저장된 selectedTab을 적용
  const selectedTab = loadSelectedTabFromLocalStorage();
